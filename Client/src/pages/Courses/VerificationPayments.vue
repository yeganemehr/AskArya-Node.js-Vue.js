		...
		<div class="row">
			<div class="col-md-8 m-md-auto col-12 card-background card-body text-center my-3">
				<div v-if="pymentStatus.loading">
					<h2 class="text-center my-3"><i class="fa fa-5x fa-spinner fa-pulse fa-fw"></i></h2>
					<p class="text-light">لطفا منتظر باشید ...</p>
				</div>
				<div v-if="pymentStatus.success">
					<h2 class="text-center my-3"><i class="fa fa-5x fa-check-circle text-success"></i></h2>
					<h3 class="mt-3 mb-2">تراکنش موفق</h3>
					<p class="text-light">خرید دوره با موفقیت انجام شد.</p>
					<p><router-link v-if="course.id" :to="`/courses/${course.slug}`">برگشت به دوره</router-link></p>
				</div>
				<div v-if="pymentStatus.error">
					<h2 class="text-center my-3"><i class="fa fa-5x fa-exclamation-circle text-danger"></i></h2>
					<h3 class="mt-3 mb-2 text-danger">تراکنش ناموفق</h3>
					<p class="text-light text-center">تراکنش شما با موفقیت پرداخت نشد.<br>درصورتی که مبلغی از حساب بانکی شما کسر شده، توسط سیستم بانکی طی 48 ساعت آینده مرجوع خواهد شد</p>
					<p><router-link v-if="course.id" :to="`/courses/${course.slug}`">برگشت به دوره</router-link></p>
				</div>
			</div>
		</div>
