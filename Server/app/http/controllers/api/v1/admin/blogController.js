...
class blogController extends controller {
	async index(req, res) {
		...
		const posts = await blogPost.paginate(
			filter,
			{
				page,
				sort: {
				...
				populate: [ { path: 'author', select: 'id name'}, { path: 'tags', select: 'name' }, { path: 'categories', select: 'name' } ],
			}
		);
		const data = {
			...posts,
	...
	async store(req, res) {
		...
		for (const tag of tags) {
			let obj = await blogTag.findOne({ name: tag }).exec();
			if (! obj) {
				obj = new blogTag({
					name: tag.trim(),
					slug: this.slug(tag),
		...
		for (const category of categories) {
			let obj = await blogCategory.findOne({ name: category }).exec();
			if (! obj) {
		...
		await newPost.save();
		res.json({
			post: this.filterPostData(newPost, await User.findById(author), tagObjects, coategoryObjects),
			status: "success",
		});
	}
	async update(req, res) {
		...
		const objForUpdate = {};
		if (req.file) {
			objForUpdate.image = this.getUrlImage(`${req.file.destination}/${req.file.filename}`);
		}
		...
		post = await blogPost.findByIdAndUpdate(req.params.id, {
			$set: { ...req.body,
				...objForUpdate
			}
		}, {
		...
		res.json({
			post: this.filterPostData(post, await User.findById(req.body.author), tagObjects, coategoryObjects),
			status: "success",
		});
	}
	...
	async upload(req, res) {
		...
		const image = this.getUrlImage(`${req.file.destination}/${req.file.filename}`);
		res.json({
			image: image,
			status: "success",
		});
